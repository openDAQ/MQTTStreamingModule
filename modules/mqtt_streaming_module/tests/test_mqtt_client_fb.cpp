...
TEST_F(MqttFbTest, DefaultMqttFbConfig)
{
    ...
    ASSERT_TRUE(types.hasKey(CLIENT_FB_NAME));
    auto defaultConfig = types.get(CLIENT_FB_NAME).createDefaultConfig();
    ASSERT_TRUE(defaultConfig.assigned());

    ASSERT_EQ(defaultConfig.getAllProperties().getCount(), 5u);

    ASSERT_TRUE(defaultConfig.hasProperty(PROPERTY_NAME_CLIENT_BROKER_ADDRESS));
    ...
    ASSERT_EQ(defaultConfig.getProperty(PROPERTY_NAME_CLIENT_BROKER_ADDRESS).getValueType(), CoreType::ctString);
    ...
    ASSERT_EQ(defaultConfig.getPropertyValue(PROPERTY_NAME_CLIENT_BROKER_ADDRESS), DEFAULT_BROKER_ADDRESS);
...
TEST_F(MqttFbTest, DISABLED_CreatingSeveralMqttFbs)
{
    const auto instance = Instance();
    daq::FunctionBlockPtr fb;
    ASSERT_NO_THROW(fb = instance.addFunctionBlock(CLIENT_FB_NAME, DaqMqttFbConfig("127.0.0.1", 1883)));
    ASSERT_EQ(fb.getStatusContainer().getStatus("ComponentStatus"),
              Enumeration("ComponentStatusType", "Ok", instance.getContext().getTypeManager()));
    daq::FunctionBlockPtr anotherFb;
    ASSERT_NO_THROW(anotherFb = instance.addFunctionBlock(CLIENT_FB_NAME, DaqMqttFbConfig("127.0.0.1", 1884)));
