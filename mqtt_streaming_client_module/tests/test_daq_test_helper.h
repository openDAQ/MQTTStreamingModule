    ...
    daq::GenericDevicePtr<daq::IDevice> DaqMqttDeviceInit(std::string connectionStr, int discoveryTimeoutMs = -1)
    {
        if (!device.assigned())
        {
            if (discoveryTimeoutMs >= 0)
            {
                ...
                auto config = module.getAvailableDeviceTypes().get(daq::modules::mqtt_streaming_client_module::DaqMqttDeviceTypeId).createDefaultConfig();
                config.setPropertyValue(daq::modules::mqtt_streaming_client_module::PROPERTY_NAME_DISCOVERY_TIMEOUT, 0);
